...
Актор отдаёт собранные карточки товаров прямо в сервис заказчика через
[MCP-коннекторы Apify](https://docs.apify.com/platform/integrations/mcp-connectors):
рабочее пространство Notion, проект Supabase, канал в мессенджере. Ключи заказчика в код
актора не попадают: площадка подставляет их на своей стороне, а актор ходит к прокси со
своим токеном прогона.

Актор намеренно не знает, куда пишет. Он спрашивает у подключённого сервиса список
инструментов, выбирает подходящий для записи, читает опубликованную схему аргументов и
подстраивает вызов под неё. Подключите Notion - создаст страницы, подключите базу -
вставит строки.
...
| Поле | Смысл |
|---|---|
| `items` | Записи карточек, передаются значениями. Прогон не может прочитать набор данных чужого прогона с обычными правами. |
| `outputConnector` | Идентификатор коннектора. В консоли отрисуется как выбор из списка. |
| `targetTable` | Имя таблицы, базы или канала. Сервисы, которым оно не нужно, его игнорируют. |
| `dryRun` | Подключиться, показать список инструментов, ничего не записывать. Так стоит начинать с новым коннектором. |

Начните с `dryRun: true`. В набор данных попадут `toolsAvailable` и схема аргументов того
инструмента, который был бы вызван - это самый быстрый способ увидеть, что ваш сервис
на самом деле умеет.

**Что выяснилось при сборке**

- `streamable_http_client` в разных версиях клиента отдаёт разное число потоков: пример в
  документации распаковывает три, установленная версия даёт два. Брать по индексу, а не
  жёсткой распаковкой.
- Имена инструментов у сервисов идут через разные разделители: `create_page` у одного,
  `notion-create-pages` у другого. Ловить по одному слову `create` нельзя - попадётся
  `create-attachment`, куда карточкам не место.
- Причина отказа пишется в набор данных, а не только в журнал: вызывающая сторона видит
  набор данных и никогда не видит журнал.
- Универсального вызова записи не существует. Notion ждёт страницы с заголовком и телом в
  Markdown, база - строки, чат - канал и текст. Актор читает схему, а не гадает по имени.

**Локальный запуск.** Коннекторы живут только в прогоне на площадке: переменной
`APIFY_MCP_PROXY_URL` на своей машине нет, и актор честно об этом сообщает вместо падения.
